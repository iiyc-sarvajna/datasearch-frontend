...
const loadProfile = async () => {
  try {
    ...
    const fileCountRes = await fetch("http://localhost:4000/my-files?q=", {
      headers: { Authorization: t },
      cache: 'no-store'
    });
 ...
 const loadMyFiles = async (search) => {
  try {
    ...
    const res = await fetch(
      `http://localhost:4000/my-files?q=${encodeURIComponent(search || "")}`,
      {
        headers: { Authorization: t },
        cache: "no-store",
      }
  ...
  const loadGlobal = async () => {
    try {
      const t = await token();
      if (!t) return;
      const res = await fetch("http://localhost:4000/search-all", {
        headers: { Authorization: t },
        cache: 'no-store'
      });
      if (res.ok) {
...
useEffect(() => {
  ...
  globalSearchTimeoutRef.current = setTimeout(async () => {
    try {
      ...
      // Search files
      const res = await fetch(
        `http://localhost:4000/search-all?q=${encodeURIComponent(globalSearch)}`,
        {
          headers: { Authorization: authToken },
          cache: "no-store",
        }
